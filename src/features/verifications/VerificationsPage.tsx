...
type VerificationRow = NonNullable<
  get200AdminVerificationsResponseJson["verifications"]
>[number];
...
const renderDocumentPreview = (fileUrl: string, verificationType?: string) => {
  if (!fileUrl)
    return (
      <Typography color="text.secondary">فایل مدرک موجود نیست.</Typography>
    );

  if (
    verificationType === "id_card_video" ||
...
export const VerificationsPage = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      clientRequest.GET("/admin/verifications", { params: { query } }),
  });

  const approveMutation = useMutation({
    mutationFn: (id: number) =>
      clientRequest.POST("/admin/verifications/{id}/approve", {
        params: { path: { id } },
      }),
    onSuccess: () => {
  ...
  const rejectMutation = useMutation({
    mutationFn: ({
      ...
      id: number;
      body: postAdminVerificationsIdRejectRequestBodyJson;
    }) =>
      clientRequest.POST("/admin/verifications/{id}/reject", {
        params: { path: { id } },
        body,
      }),
    onSuccess: () => {
  ...
  const columns: GridColDef<VerificationRow>[] = [
    ...
    { field: "user_id", headerName: "شناسه کاربر", width: 120 },
    {
      ...
      width: 150,
      valueGetter: (_, row) =>
        verificationTypeLabel[row.verification_type ?? ""] ??
    ...
    },
    {
      ...
      headerName: "دلیل رد",
      width: 180,
      valueGetter: (_, row) => row.failure_reason ?? "-",
    },
    { field: "created_at", headerName: "ایجاد", width: 130 },
    {
      ...
      renderCell: (p) => (
        <Stack direction="row" spacing={1}>
          <Button size="small" onClick={() => setDrawer(p.row)}>
            مشاهده
          </Button>
          <Button
            size="small"
            color="success"
            onClick={() => approveMutation.mutate(p.row.id!)}
          >
            تایید
          </Button>
          <Button
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="مشاهده مدارک، پیش‌نمایش، تایید/رد با دلیل"
      />
      <Stack mb={2}>
        <TextField
          select
        ...
        <Stack p={2.5} spacing={2} width={{ xs: 380, md: 560 }}>
          ...
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 2 }}>
            ...
            >
              ...
              <Chip
                label={`نوع: ${verificationTypeLabel[drawer?.verification_type ?? ""] ?? drawer?.verification_type ?? "-"}`}
              />
              <Chip
                label={`وضعیت: ${drawer?.status ?? "-"}`}
                color={
          ...
          <Paper variant="outlined" sx={{ p: 1.5, borderRadius: 2 }}>
            {renderDocumentPreview(
              drawerFileUrl,
              drawer?.verification_type ?? undefined,
            )}
          </Paper>
        </Stack>
      </Drawer>
      ...
      <FormModal
        ...
        onSubmit={async () => {
          ...
          await rejectMutation.mutateAsync({
            id: rejectId,
            body: { failure_reason: reason.trim() },
          });
        }}
      >
